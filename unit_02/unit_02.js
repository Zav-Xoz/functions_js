...
function t1(min, max) {
    ...
    document.querySelector('.out-1').innerHTML = Math.floor(rand);
}
...
function t2(min, max, block) {
    let rand = min + Math.random() * (max + 1 - min);
    block.innerHTML = Math.floor(rand);
}
...
function t3(min = 0, max = 100) {
    let rand = min + Math.random() * (max + 1 - min);
    document.querySelector('.out-3').innerHTML = Math.floor(rand);
}
...
function t12(...array) {
    out12.innerHTML = array.reduce((accum, item) => accum + item);
}
