...
function t3() {
    let ch = document.querySelector('.ch-3');
    if (ch.checked) return ch.value;
    return false;
}
...
function t4() {
    const elements = document.querySelectorAll('.r-4');
    let out;
...
function t21(s) {
    while (true) {
        const value = t13(10, 20);
        // console.log(value)
        if (value % 2 === 0 && s === 'even') return value;
        if (value % 2 !== 0 && s === 'odd') return value;
    }
}
...
function t27(v) {
    ...
    const out = Object.keys(obj27).find(item => obj27[item] === v);
